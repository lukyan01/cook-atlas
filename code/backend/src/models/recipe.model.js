...
class RecipeModel {
  ...
  async getRecipesByCreator(userId) {
    const result = await getPool().query(
      "SELECT * FROM recipes WHERE creator_id = $1 ORDER BY recipe_id DESC",
      [userId]
    );
    return result.rows;
  }
  ...
  async searchRecipes(params) {
    ...
    // Text search
    if (query) {
      sql += ` AND (title ILIKE $${paramIndex} OR description ILIKE $${
        paramIndex + 1
      })`;
  ...
  async createRecipe(recipeData) {
    ...
    const result = await getPool().query(
      `INSERT INTO recipes (creator_id, title, description, cook_time, prep_time,
