...
class UserModel {
    ...
    async registerUser({username, email, password, role = 'registered'}) {
        ...
        const usernameCheck = await getPool().query('SELECT * FROM users WHERE username = $1', [username]);
    ...
    async loginUser({email, password}) {
        ...
        const passwordMatch = await bcrypt.compare(String(password), String(user.password_hash));
