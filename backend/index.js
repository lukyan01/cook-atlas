...
app.post('/recipes', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `INSERT INTO recipes (creator_id, title, description, cook_time, prep_time, skill_level, source_platform,
                                  source_url)
...
app.post('/auth/login', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('SELECT user_id, username, email, role, password FROM users WHERE email = $1',
            [email]);
