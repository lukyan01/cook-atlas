...
function AdminPage() {
  ...
  const fetchRecipes = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:3000/recipes?timestamp=${Date.now()}`
      );
  ...
  const handleUpdate = async () => {
    try {
      await fetch(`http://localhost:3000/recipes/${updateId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: updateTitle,
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`http://localhost:3000/recipes/${deleteId}`, {
        method: "DELETE",
      });
      ...
      <button
        onClick={async () => {
          try {
            ...
            const res = await fetch(
              `http://localhost:3000/recipes/search?${params}`
            );
            const data = await res.json();
